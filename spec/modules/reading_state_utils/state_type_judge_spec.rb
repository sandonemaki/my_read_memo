RSpec.describe ReadingStateUtils do
  describe ReadingStateUtils::StateTypeJudge do
    describe '.determine_state' do
      let(:book) { Book.create(title: 'ダイニングタイトル', author_1: '東野智子', total_page: 80, reading_state: 0) }

      context 'さらさら読書画像メモが0の場合' do
        before do
      ...
      context 'さらさら読書画像メモが総ページに対して1/8未満の場合' do
        before do
          @already_read_count = 0
          @unread_count = 9
          @total_page = 80
      ...
      context 'さらさら読書画像メモが総ページに対して1/8以上で、未読が総ページに対して1/4以上の場合' do
        before do
          @already_read_count = 0
          @unread_count = 20
          @total_page = 80
      ...
      context 'さらさら読書画像メモが総ページに対して1/8以上で、未読が総ページに対して0以上1/4未満の場合' do
        before do
          @already_read_count = 0
          @unread_count = 19
          @total_page = 80
      ...
      context 'さらさら読書画像メモが総ページに対して1/8以上で、未読が0の場合' do
        before do
